-- based on Mike Spivey's Parsing from the POPL course
...
module Parsing(nextch, switch, star, incln, make_kwlookup,
		...
		dialog, dialogm, printStrLn, showlist, joinwith, Lexer,
		Parser, Syntax) where
...
data LexBuf t = MkLexBuf Int [LexTok t] [LexTok t] Int deriving Show
...
data Result t w = 
    Success w (LexBuf t)
  | Failure (LexBuf t)
  | More ([LexTok t] -> Result t w) deriving Show
